...
from llambo.gemma_adapter import GemmaLocalAdapter # <-- ADDED IMPORT (Adjust path if needed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--acq_strategy', type=str)
    ...
    print(f"Log path: {logging_fpath}")
    setup_logging(logging_fpath)

    tot_llm_cost = 0

    # it's not adviced to instantiate the LLM all the time in the loop 
    # therefore we do it outside the loop 
    try:
        adapter_instance = GemmaLocalAdapter(
            model_name="google/gemma-2-2b-it" # Hardcode default for now
            # Add other necessary args if your adapter's __init__ requires them
        )
        logger.info(f"Instantiated GemmaLocalAdapter with model: {adapter_instance.model_name}")
    except Exception as e:
        logger.error(f"Failed to instantiate GemmaLocalAdapter: {e}")
    ...
    for seed in seeds_to_run:
        ...
        llambo = LLAMBO(
            task_context, 
            sm_mode, 
            n_candidates=5, #initially 10 
            n_templates=2, 
            n_gens=5,  #initially 10             
            alpha=0.1, 
            n_initial_samples=5, 
            n_trials=100,  # originally it was 25  
            init_f=benchmark.generate_initialization,
            bbox_eval_f=benchmark.evaluate_point, 
            llm_adapter=adapter_instance, # just insert the adapter
            # chat_engine=chat_engine, # we don't have a chat engine in our adapter version 
            use_input_warping=use_input_warping, 
            sm_acquisition_strategy=acq_strategy
        )
        ...
        configs, fvals = llambo.optimize(
            seed = seed, 
            save_res_dir=save_res_dir,   # saving during trials in case of possible crashes; useful for resuming the run
            resume=resume
        )
        ...
        search_info = {
            ...
            'acq_attention_results': llambo.acq_func.acq_attention_results, # Adapter now only stores ACQ results
            'sm_attention_results': llambo.surrogate_model.sm_attention_results # Get SM results directly
        }
